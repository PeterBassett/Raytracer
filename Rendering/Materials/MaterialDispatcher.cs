...
namespace Raytracer.Rendering.Materials
{
    class MaterialDispatcher
    {
        ...
        public Vector2D InterpolateTextureUV(Vector3D pointOfIntersection, Vector3D[] vertexes, Vector2D[] textureUVs)
        {
        ...
        {
            var uAxis = new Vector3D(plane.Normal.Y, plane.Normal.Z, -plane.Normal.X);
            var vAxis = Vector3D.CrossProduct(uAxis, plane.Normal);

            Material.CloneElements(output, material);

            var u = Vector3D.DotProduct(info.HitPoint, uAxis) * material.UScale;
            var v = Vector3D.DotProduct(info.HitPoint, vAxis) * material.VScale;
